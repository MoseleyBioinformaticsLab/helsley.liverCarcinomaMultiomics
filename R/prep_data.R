prep_rna_data_patient = function(
	rna_data = rna_data,
	...
	gene_ranges = GRanges(
		seqnames = gene_metadata$gene_chr,
		ranges = IRanges(
			start = gene_metadata$gene_start,
	...
	rna_dds = DESeq2::DESeqDataSetFromMatrix(
		rna_matrix,
		sample_metadata,
...
prep_rna_data_treatment = function(
	rna_data = rna_data,
	sample_metadata = sample_info,
	count_locs = seq(2, 16),
	feature_metadata = seq(17, 25)
...
setup_metabolomics = function(
	...
	# tar_source("R")
	# tar_load(primary_metabolism_file)
	# all_data = suppressMessages(readxl::read_excel(primary_metabolism_file,
	# sheet = "data",
	# skip = 8)) |>
	# janitor::clean_names()  |>
	# rename_experimental_samples()
	# tar_load(sample_info)
	# metabolite_type = "primary_metabolism"
	...
	if ("bin_base_name" %in% colnames(all_data)) {
		feature_id = paste0(
			janitor::make_clean_names(all_data[["bin_base_name"]]),
			".",
			metabolite_type
		)
		...
		feature_id = paste0(
			janitor::make_clean_names(all_data[["identifier"]]),
			".",
			metabolite_type
		)
	...
	# }
	sample_info_extra$treatment = factor(
		sample_info_extra$treatment
	)
	sample_info_extra$patient = factor(sample_info_extra$patient)

	out_data = DESeq2::DESeqDataSetFromMatrix(
		sample_matrix,
		sample_info_extra,
		design = ~treatment
	)
...
add_blanks_pooled = function(sample_names, sample_info) {
	# sample_names = colnames(cor_pca$correlation$cor)
	extra_samples = tibble::tibble(
		sample_id = setdiff(sample_names, sample_info$sample_id)
	) |>
		dplyr::mutate(
			treatment = dplyr::case_when(
	...
	sample_info = dplyr::bind_rows(sample_info, extra_samples)
	sample_info
}
...
calculate_patient_ratios = function(normalized_data, sample_info) {
	# normalized_data = tar_read(rna_keep)
	# tar_load(sample_info)
	# sample_info = sample_info |> dplyr::filter(!sample_id %in% "s11")
	intersect_samples = base::intersect(
		unique(normalized_data$sample_id),
		sample_info$sample_id
	)
	matched_sample_info = determine_matched_samples(
		sample_info |>
			dplyr::filter(sample_id %in% intersect_samples)
	)
	normalized_data = dplyr::inner_join(
		normalized_data,
		matched_sample_info[, c("patient", "sample_id", "treatment")],
		by = "sample_id"
	)
	patient_ratios = normalized_data |>
		tidyr::pivot_wider(
			id_cols = c("patient", "feature_id"),
			names_from = "treatment",
			values_from = "abundance"
		) |>
		dplyr::mutate(abundance = cancerous / normal_adjacent, sample_id = patient)
	patient_ratios
}
...
split_intensities_feature_metadata = function(all_data) {
	...
	list(abundance = sample_data, metadata = metadata)
}
