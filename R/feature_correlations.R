feature_correlations = function(
	...
	use_comparison = tidyr::expand_grid(
		rna = rna_id,
	...
	out_cor = switch(
		method,
		icikt = {
			all_cor = ICIKendallTau::ici_kendalltau(
				t(all_counts),
				global_na = 0,
...
find_rna_metabolite_pairs = function(
	rna_de_patient,
	...
	keep_genes = purrr::map(
		rna_compounds_matrix[c("lipids", "compounds")],
		rownames
	) |>
		unlist(use.names = FALSE) |>
		unique()
	keep_metabolites = purrr::map(
		rna_compounds_matrix[c("lipids", "compounds")],
		colnames
	) |>
		unlist(use.names = FALSE) |>
	...
	rna_de_spearman = dplyr::inner_join(
		trim_rna_de_patient,
		trim_spearman_sig,
		suffix = c(".de", ".cor"),
...
bind_metabolomics_counts = function(
	bioamines_collapsed,
	lipidomics_collapsed,
	pm_collapsed,
	matched_samples
	...
	all_norm = rbind(bioamines_norm, lipidomics_norm, pm_norm)
	all_rows = rbind(bioamines_rows, lipidomics_rows, pm_rows)
	all_cols = rbind(bioamines_cols, lipidomics_cols, pm_cols)

	all_collapsed = SummarizedExperiment(
		assays = SimpleList(counts = all_norm),
		rowData = all_rows,
		colData = pm_cols
	)
...
get_rna_norm_values = function(rna_collapsed, matched_samples) {
	# tar_load(c(rna_collapsed,
	# 				 matched_samples))
	...
	rna_norm = SummarizedExperiment(
		assays = SimpleList(counts = norm_counts),
		colData = rna_cols,
...
rna_within_correlation = function(
	rna_collapsed,
	rna_significant,
	matched_samples,
	method = "icikt"
...
metabolites_within_correlation = function(
	bioamines_collapsed,
	lipidomics_collapsed,
	pm_collapsed,
	matched_samples,
	...
	all_counts = rbind(bioamines_counts, lipidomics_counts, pm_counts)
	out_cor = switch(
		method,
		icikt = {
			all_cor = ICIKendallTau::ici_kendalltau(
...
run_correlation = function(
	...
	named_comparisons = data.frame(
		s1 = colnames(data_matrix)[pairwise_comparisons[1, ]],
	...
	if (!is.null(include_only)) {
		if (is.character(include_only) || is.numeric(include_only)) {
			...
			if (length(include_only) == 2) {
				...
				l1_include = (named_comparisons$s1 %in% include_only[[1]]) |
					(named_comparisons$s2 %in% include_only[[1]])
				l2_include = (named_comparisons$s1 %in% include_only[[2]]) |
					(named_comparisons$s2 %in% include_only[[2]])
				named_comparisons = named_comparisons[
					(l1_include & l2_include),
				]
			} else {
				stop(
					"include_only must either be a single vector, or a list of 2 vectors!"
				)
			}
		}
	}
	...
	if (nrow(named_comparisons) == 0) {
		stop(
			"nrow(named_comparisons) == 0, did you create include_only correctly?"
		)
	}
	...
	do_split = function(do_comparisons, data_matrix, method, use) {
		...
		for (irow in seq_len(nrow(do_comparisons))) {
			...
			n_match = sum(
				!is.na(data_matrix[, iloc]) & !is.na(data_matrix[, jloc])
			)
			if (n_match >= 4) {
				cor_res = cor.test(
					data_matrix[, iloc],
...
just_rna_abundances = function(rna_collapsed, matched_samples) {
	rna_counts = assays(rna_collapsed)$counts[, matched_samples]
	rna_counts
}

just_metabolite_abundances = function(
	bioamines_collapsed,
	lipidomics_collapsed,
	pm_collapsed,
	matched_samples
...
check_metabolite_correlations = function(
	...
	correlations = purrr::imap(split_features, \(in_features, id) {
		tmp_cor = metabolites_within_cor |>
			dplyr::filter(
				((s1 %in% in_features[1]) & (s2 %in% in_features[2])) |
					((s1 %in% in_features[2]) & (s2 %in% in_features[1]))
			)
...
find_genes_correlated_lipids = function(
	...
	annotated_lipids = metabolomics_enrichment_lipid_binomial$enrichment@annotation@annotation_features[
		sig_binomial_id
	]
	...
	out_genes = purrr::imap(annotated_lipids, \(lipids, id) {
		...
		lipid_cor = sig_cor |>
			dplyr::filter(
				metabolite %in% lipid_direction,
				transcript %in% sig_rna$feature_id
			)
		if (nrow(lipid_cor) > 0) {
...
create_rna_compounds_matrix = function(
	compounds = rna_correlated_interesting_compounds,
	...
	rna_annotation = rna_annotation |>
		dplyr::transmute(s1 = ensembl_gene_id, symbol = hgnc_symbol) |>
		dplyr::distinct()

	compound_annotation = compound_annotation |>
		dplyr::transmute(s2 = feature_id, metabolite = metabolite_id) |>
		dplyr::distinct()

	compound_df = all_correlation |>
		dplyr::filter(
			s1 %in% unique(all_compounds$transcript),
			s2 %in% unique(all_compounds$metabolite)
		)

	compound_matrix = ICIKendallTau::long_df_2_cor_matrix(
		compound_df,
		is_square = FALSE
	)
	...
	lipid_df = all_correlation |>
		dplyr::filter(
			s1 %in% unique(all_lipids$transcript),
			s2 %in% unique(all_lipids$metabolite)
		)

	lipid_matrix = ICIKendallTau::long_df_2_cor_matrix(
		lipid_df,
		is_square = FALSE
	)
		...
		dplyr::filter(
			feature_id %in%
				c(
					...
					rownames(compound_matrix),
					colnames(lipid_matrix),
					colnames(compound_matrix)
				)
		)

	list(
		compounds = compound_matrix,
		lipids = lipid_matrix,
		labels = all_labels
	)
}
