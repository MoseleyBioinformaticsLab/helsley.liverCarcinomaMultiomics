	...
	de_data = de_data |>
		dplyr::mutate(
			fc_dir = dplyr::case_when(
				padj <= 0.01 & log2FoldChange > 0 ~ "positive",
				padj <= 0.01 & log2FoldChange < 0 ~ "negative",
				TRUE ~ "none"
			)
		)
	out_plot = de_data |>
		...
		geom_point(
			data = de_data |> dplyr::filter(padj <= 0.01),
			aes(color = fc_dir)
		) +
...
generate_metabolomics_de_output = function(
	...
	data_dictionary = tibble::tribble(
		...
		"log2FoldChange"                                                                       ,
		"Fold change of the contrast / comparison calculated by DESeq2"                        ,
		"lfcSE"                                                                                ,
...
generate_transcriptomics_de_output = function(
	...
	data_dictionary = tibble::tribble(
		...
		~meaning                                                      ,
		"baseMean"                                                    ,
		"mean across reference samples calculated by DESeq2"          ,
		"log2FoldChange"                                              ,
		"Fold change of contrast / comparisons calculated by DESeq2"  ,
		"lfcSE"                                                       ,
		"standard error of fold change"                               ,
		"stat"                                                        ,
		"test statistic"                                              ,
