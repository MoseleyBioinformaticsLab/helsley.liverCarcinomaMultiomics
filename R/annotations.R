get_ensembl_uniprot = function(version = "111") {
	...
	feature_data = getBM(attributes = use_attributes, mart = ensembl)
...
create_reactome_gene_annotations = function(
	reactome_file,
	...
	out_annotation = categoryCompare2::annotation(
		annotation_features = split_reactome,
...
create_reactome_chebi_annotations = function(
	chebi_reactome_file,
...
create_go_annotations = function(ensembl_uniprot, go_file, namespace_file) {
	# tar_load(ensembl_uniprot)
	# tar_load(go_file)
	# tar_load(namespace_file)
	feature_translation = ensembl_uniprot |>
		dplyr::transmute(from = uniprot_gn_id, to = ensembl_gene_id) |>
		dplyr::filter(nchar(from) > 0, nchar(to) > 0) |>
		dplyr::distinct()

	out_annotation = gocats_to_annotation(
		go_file,
		namespace_file,
		feature_type = "ensembl_gene",
...
group_annotations = function(group_enrichment, similarity_cutoff = 0.8) {
	...
	enrich_comm_labels = label_communities(
		enrich_communities,
		just_enrich@annotation
	)

	enrich_table = table_from_graph(
		enrich_graph,
		enrich_assign,
		enrich_comm_labels
	)
...
group_annotations_each = function(group_enrichment, similarity_cutoff = 0.8) {
...
run_enrichment = function(
	...
	enrich_list = list(all = enrich_all, pos = enrich_pos, neg = enrich_neg)
	stats_list = list(all = stats_all, pos = stats_pos, neg = stats_neg)
	...
	enrich_sig = get_significant_annotations(
		enrich_comb,
		padjust <= 0.01,
		counts >= 2,
		counts <= 500
	...
	return(list(enrich = enrich_sig, stats = stats_list))
}

map_chebi_to_inchikey = function(
	inchi_df,
...
write_inchi = function(chebi_inchi_file, inchi_dir = "data/chebi") {
...
get_inchikey_hash = function(inchi_df, inchikey_directory = "data/chebi") {
	all_files = paste(
		sort(dir(inchikey_directory, pattern = "*.inchi*")),
		collapse = ":"
	)
...
write_goeach_to_excel = function(go_stuff, reactome_stuff) {
	...
	tab_out = c(list(dictionary = data_dictionary), all_out)
...
find_interesting_gm_groups = function(
	...
	group_genes_metabolites = purrr::map(split_go, \(in_go) {
		...
		metabolite_cor = metabolites_within_cor |>
			dplyr::filter(
				(s1 %in% unique(gene_metabolite$metabolite)) &
					(s2 %in% unique(gene_metabolite$metabolite))
			)
...
replace_lipid_ids = function(metabolomics_feature_list) {
	...
	tmp_data = dplyr::left_join(
		metabolomics_feature_list,
		fixed_lipids,
		by = c("metabolite_id" = "lipid"),
		relationship = "many-to-many"
	)
	tmp_data = tmp_data |>
		dplyr::mutate(
			metabolite_id2 = dplyr::case_when(
...
annotate_lipids = function(metabolomics_feature_list) {
	...
	lipids_2_annotation = dplyr::left_join(
		metabolomics_feature_list[, c("feature_id", "metabolite_id")],
		out_data,
		by = c("metabolite_id" = "lipid"),
		relationship = "many-to-many"
	)
	...
	annotation_list = purrr::map(annotation_cols, \(in_col) {
		tmp_list = split(
			lipids_2_annotation$feature_id,
			lipids_2_annotation[[in_col]]
		)
	...
	tg_list_annot = list(
		`class_total_db:TG_lte_2` = tg_annotations |>
			dplyr::filter(total_db <= 2) |>
			dplyr::pull(feature_id),
	...
	lipid_annotation = categoryCompare2::annotation(
		feature_annotations,
		annotation_type = "lipids",
		feature_type = "lipids"
	)
...
fix_lipids = function(lipid_ids) {
	...
	original_ids = data.frame(lipid = lipids_nona, pass1 = lipid_ids2)
	...
	merged_df = data.frame(lipid_id = names(merged), Molecule = merged)
...
rmf_annotate_lipids = function(in_lipids) {
	...
	should_none_l1 = is_annotated_should_multi$l_1 ==
		is_annotated_should_multi$total_cl
	...
	out_annotated = dplyr::bind_rows(
		is_annotated_should_multi,
...
annotate_metabolite_type = function(metabolomics_feature_list) {
	# tar_load(metabolomics_feature_list)
	split_type = split(
		metabolomics_feature_list$feature_id,
		metabolomics_feature_list$type
	)
	split_type = purrr::map(split_type, unique)
	annotation_type = categoryCompare2::annotation(
		split_type,
...
run_binomial = function(metabolomics_de_patient_list, lipid_annotations) {
	...
	binomial_res = binomial_feature_enrichment(
		new(
			"binomial_features",
			positivefc = pos_features,
			negativefc = neg_features,
			annotation = lipid_annotations
		),
		p_adjust = "BH",
		min_features = 6
	)
	...
	list(enrichment = binomial_res, stats = res_df)
}

enrich_genes_correlated_lipids = function(
	...
	enrich_each = purrr::map(
		...
		\(in_group) {
			...
			list(enrich = tmp_enrich, stats = tmp_df)
		}
	)
}

binomial_genes_correlated_lipids = function(
	rna_correlated_interesting_lipids,
	rna_de_patient,
	ensembl_go
) {
	...
	group_transcripts = purrr::map(
		rna_correlated_interesting_lipids$groups,
		\(in_group) {
			unique(in_group$transcript)
		}
	)
	group_annotations = categoryCompare2::annotation(
		group_transcripts,
		annotation_type = "lipidGroup",
	...
	group_binomial_stats = group_binomial@statistics@statistic_data |>
		as.data.frame()
	group_binomial_stats$id = rownames(group_binomial_stats)

	go_binomial = purrr::map(
		rna_correlated_interesting_lipids$groups,
		\(in_group) {
			...
			tmp_df$id = rownames(tmp_df)
			tmp_df$description = ensembl_go@description[tmp_df$id]
			list(rna_lfc = lfc_values, enrich = binom_enrich, stats = tmp_df)
		}
	)
	...
	list(
		groups = list(
			interesting = rna_correlated_interesting_lipids,
...
map_features_kegg_chebi = function(
	metabolomics_feature_list,
	inchikey_kegg,
	chebi_inchikey
) {
	...
	feature_other = dplyr::left_join(
		feature_other,
		chebi_inchikey[, c("chebi_id", "in_ch_i_key")],
