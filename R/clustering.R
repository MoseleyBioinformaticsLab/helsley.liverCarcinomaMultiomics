...
label_clusters_second_pass = function(
	cluster_info,
	hclust_obj,
	sub_cluster,
	n_sub
	...
	sub_color = cluster_info |>
		...
		dplyr::pull(color) |>
		unique()
	new_colors = monochromeR::generate_palette(
		sub_color,
...
create_lipid_labels = function(feature_lipid) {
	...
	lipid_annotation_df = purrr::imap(
		feature_lipid_list,
		\(in_feature, in_annotation) {
	...
	lipid_out_annotation = dplyr::left_join(
		lipid_class,
		lipid_length,
		by = "feature_id"
	)
...
cluster_create_heatmaps = function(
	...
	gene_order = tibble::tibble(
		feature_id = gene_dist$labels,
		order = order(gene_dist$order)
	)
	lipid_order = tibble::tibble(
		feature_id = lipid_dist$labels,
		order = order(lipid_dist$order)
	)
	lipid_clusters = dplyr::left_join(
		lipid_clusters,
		lipid_order,
		by = "feature_id"
	)
	...
	full_row_annotation = ComplexHeatmap::HeatmapAnnotation(
		df = gene_clusters |> dplyr::transmute(gene = cluster),
		col = gene_colors,
		which = "row",
		show_annotation_name = FALSE
	)
	full_col_annotation = ComplexHeatmap::HeatmapAnnotation(
		df = lipid_clusters |> dplyr::transmute(metabolite = cluster),
		col = lipid_colors,
		which = "column"
	)
	cor_map = circlize::colorRamp2(
		seq(-1, 1, length.out = 20),
		scico::scico(20, palette = "vik")
	)
	full_lipid_map = ComplexHeatmap::Heatmap(
		lipid_matrix,
		col = cor_map,
		name = "Spearman",
	...
	lipid_gene_heatmaps = purrr::imap(
		split_gene_clusters,
		\(in_cluster, cluster_id) {
			# in_cluster = split_gene_clusters[[5]]
			# cluster_id = names(split_gene_clusters)[5]
			non_cluster = gene_dist$labels[
				!(gene_dist$labels %in% in_cluster$feature_id)
			]
			logical_non_cluster = !(gene_dist$labels %in% in_cluster$feature_id)

			tmp_gene_annote = ComplexHeatmap::HeatmapAnnotation(
				df = in_cluster |> dplyr::transmute(gene = cluster),
				col = gene_colors,
				which = "row",
				show_annotation_name = FALSE
			)
			...
			out_heatmap = ComplexHeatmap::Heatmap(
				subset_matrix,
				col = cor_map,
				name = "Cor",
	...
	return(list(
		heatmaps = out_gene_heatmaps,
		clusters = list(genes = gene_clusters, metabolites = lipid_clusters)
	))
}

write_lipid_heatmaps_includes = function(
	plot_list,
...
write_compound_heatmaps_includes = function(
	plot_list,
...
cluster_add_feature_info = function(
	clusters,
	rna_list,
	metabolites_list,
	feature_lipid
	...
	gene_data = dplyr::left_join(clusters$genes, rna_list, by = "feature_id") |>
		dplyr::arrange(order)
	metabolites_data = dplyr::left_join(
	...
	frac_lipids = sum(grepl("lipids", metabolites_data$feature_id)) /
		nrow(metabolites_data)
	...
	return(list(genes = gene_data, metabolites = metabolites_data))
}
