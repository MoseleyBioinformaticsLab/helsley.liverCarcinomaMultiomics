...
rna_large_cor = rna_within_cor$cor |>
	dplyr::filter((s1 %in% names(large_genes)) & (s2 %in% names(large_genes)))

metabolite_large_cor = metabolites_within_cor$cor |>
...
```{r}
#| label: check_missing
all_genes = unique(unlist(rna_within_cor$cor[, c("s1", "s2")]))
