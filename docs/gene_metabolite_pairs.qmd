...
We can also compare the variance log-ratios to the log-fold-changes of cancer / normal-adjacent, as shown in @fig-variance-sd-lfc and @fig-variance-rsd-lfc.
In these cases, the variance ratios for the **transcriptomics** standard deviations are positively correlated with log-fold-change, but negatively for the rest, and then mostly flat for relative standard deviation.

```{r}
#| label: fig-variance-sd-lfc
#| fig-cap: Log-ratio of standard-deviations to log-fold-changes of cancer / normal-adjacent.
rna_var_lfc = dplyr::left_join(rna_variances, rna_de_patient[, c("log2FoldChange", "feature_id")], by = "feature_id")

rna_sd_lfc_plot = rna_var_lfc |>
	dplyr::filter(which %in% "sd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Transcriptomics", y = "log10-SD(cancerous / normal-adjacent)")
lipidomics_var_lfc = dplyr::left_join(lipidomics_variances, metabolomics_de_patient_list[, c("log2FoldChange", "feature_id")], by = "feature_id")
lipidomics_sd_lfc_plot = lipidomics_var_lfc |>
	dplyr::filter(which %in% "sd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Lipidomics", y = "log10-SD(cancerous / normal-adjacent)")

pm_var_lfc = dplyr::left_join(pm_variances, metabolomics_de_patient_list[, c("log2FoldChange", "feature_id")], by = "feature_id")
pm_sd_lfc_plot = pm_var_lfc |>
	dplyr::filter(which %in% "sd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Primary-Metabolism", y = "log10-SD(cancerous / normal-adjacent)")

bioamines_var_lfc = dplyr::left_join(bioamines_variances, metabolomics_de_patient_list[, c("log2FoldChange", "feature_id")], by = "feature_id")
bioamines_sd_lfc_plot = bioamines_var_lfc |>
	dplyr::filter(which %in% "sd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Bioamines", y = "log10-SD(cancerous / normal-adjacent)")

(rna_sd_lfc_plot | bioamines_sd_lfc_plot) / (lipidomics_sd_lfc_plot | pm_sd_lfc_plot)
```

```{r}
#| label: fig-variance-rsd-lfc
#| fig-cap: Log-ratio of relative-standard-deviations to log-fold-changes of cancer / normal-adjacent.
rna_rsd_lfc_plot = rna_var_lfc |>
	dplyr::filter(which %in% "rsd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Transcriptomics", y = "log10-RSD(cancerous / normal-adjacent)")
lipidomics_rsd_lfc_plot = lipidomics_var_lfc |>
	dplyr::filter(which %in% "rsd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Lipidomics", y = "log10-RSD(cancerous / normal-adjacent)")

pm_rsd_lfc_plot = pm_var_lfc |>
	dplyr::filter(which %in% "rsd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Primary-Metabolism", y = "log10-RSD(cancerous / normal-adjacent)")

bioamines_rsd_lfc_plot = bioamines_var_lfc |>
	dplyr::filter(which %in% "rsd") |>
	ggplot(aes(x = log2FoldChange, y = ratio)) +
	geom_point() +
	labs(subtitle = "Bioamines", y = "log10-RSD(cancerous / normal-adjacent)")

(rna_rsd_lfc_plot | bioamines_rsd_lfc_plot) / (lipidomics_rsd_lfc_plot | pm_rsd_lfc_plot)
```
...
* So this makes a much simpler story as far as what should be plotted in association with the heatmaps.
* If we want to really talk about genes with positive log-fold-changes and positive correlation with metabolites, then we can't restrict it to differentially expressed genes.