...
tar_load(c(
  ...
  compound_kegg,
  metabolomics_de_patient_kegg,
  metabolites_multi_assay_correlations
))
...
Ensembl gene IDs were mapped to Uniprot and Entrez gene IDs using version 111 of Ensembl annotations, accessed via the {biomaRt} package.
Uniprot genes were remapped to GO terms with consideration of *is_a*, *part_of* and *has_part*, with proper handling of the directionality of the *has_part* relationships using the {gocats} python package.

West Coast Metabolomics provides InChiKey IDs for their identified compounds, providing the ability to map them to other databases.
For reactome pathways, which provides ChEBI to reactome pathways, all ChEBI compound InChI strings (downloaded from ChEBI) were converted to InChiKey's using OpenBabel chemical conversion software (v 3.1.1), and then we could complete mapping from InChiKey to reactome pathway.
For KEGG pathways, we used translations of InChiKey to KEGG compounds provided by the MetaCyc translation service.

Lipid features such as lipid class, chain length, and unsaturation were provided by the {lipidr} package after cleaning the lipid identifier strings.
...
patient_treatment = dplyr::left_join(
  rna_de_patient[, c("feature_id", "pvalue", "padj")],
  rna_de_treatment[, c("feature_id", "pvalue", "padj")],
  by = "feature_id",
  suffix = c(".patient", ".treatment")
)
patient_treatment = dplyr::left_join(
  patient_treatment,
  novagene_rna_stats[, c("gene_id", "pvalue", "padj")],
  by = c("feature_id" = "gene_id"),
  suffix = c(".patient", ".novagene")
)
patient_treatment = patient_treatment |>
  dplyr::mutate(pvalue.novagene = pvalue, padj.novagene = padj) |>
  dplyr::mutate(
    patient_treatment = -1 *
      log10(pvalue.patient) -
      -1 * log10(pvalue.treatment),
    treatment_novagene = -1 *
      log10(pvalue.treatment) -
      -1 * log10(pvalue.novagene),
...
patient_treatment_pvals = patient_treatment |>
  ...
  dplyr::mutate(
    type = factor(
      type,
      levels = c("novagene", "treatment", "patient"),
      ordered = TRUE
    )
  )
...
patient_treatment_diff = patient_treatment |>
  ...
  dplyr::mutate(
    comparison = factor(
      comparison,
      levels = c("patient_novagene", "treatment_novagene", "patient_treatment"),
      ordered = TRUE
    )
  )
...
transcriptomics_sig = list(
  patient = patient_treatment |>
...
metabolomics_diff_summary = metabolomics_diffs |>
  dplyr::group_by(type) |>
  dplyr::summarise(
    `Patient Worse` = sum(difference <= 0),
...
bioamines_compare = list(
  patient = diff_split_type$bioamines |>
    dplyr::filter(padj.patient <= 0.05) |>
    dplyr::pull(feature_id),
...
ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(bioamines_compare),
  column_title = "Bioamines"
)
```
...
lipidomics_compare = list(
  patient = diff_split_type$lipidomics |>
    dplyr::filter(padj.patient <= 0.05) |>
    dplyr::pull(feature_id),
...
ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(lipidomics_compare),
  column_title = "Lipidomics"
)
```
...
ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(pm_compare),
  column_title = "Primary Metabolism"
)
```

## Multi-Assay Metabolite Comparisons

WCMC has several metabolites that are measured in multiple assays of *primary metabolism*, *biogenic amines (bioamines)* and *lipidomics*.
Ideally, we would expect that the same metabolites measures in multiple assays would be highly correlated.
A histogram of Spearman correlations are shown in @fig-multi-assay-correlations, with lines indicating those correlations with raw or adjusted p-values less than 0.05.
This shows that significant correlations do have relatively high correlation values.

```{r}
#| label: fig-multi-assay-correlations
#| fig-cap: Metabolite - metabolite Spearman correlations for those metabolites measured in multiple measurement or assay types. Blue line indicates those correlations with a p-value < 0.05, and the red line indicates those with an adjusted p-value < 0.05.
p_limit = metabolites_multi_assay_correlations |>
  dplyr::filter(pvalue <= 0.05) |>
  dplyr::slice_min(cor, n = 1) |>
  dplyr::pull(cor)
padjust_limit = metabolites_multi_assay_correlations |>
...
metabolites_multi_assay_correlations |>
  ...
  geom_vline(xintercept = p_limit[1], color = "blue") +
  geom_vline(xintercept = padjust_limit[1], color = "red") +
  labs(x = "Metabolite - metabolite Spearman correlations")
```

## Enrichment Results

For hypergeometric enrichment of the metabolites to work, we need the significant metabolites to have some kind of identifier outside of the WCMC ID in an existing database or pathway annotations.
Simply looking at whether they are named, and then if they map to a pathway can help determine if any useable results will be found using hypergeometric enrichment.
@tbl-named-metabolite-locs shows that there are very few named metabolites in general, and in the significant set.
@tbl-bioamines-pathway shows that even amongst the bioamines, the set with the most significant and named entries, there is only a small fraction with annotations to pathways.
...
metabolites_significant = metabolomics_de_patient_list |>
  dplyr::mutate(
    has_name = dplyr::case_when(
      !is.na(metabolite_id) ~ "named",
      is.na(metabolite_id) ~ "unknown"
    ),
...
bioamines_sig = metabolites_significant |>
  ...
  dplyr::select(feature_id, metabolite_id, log2FoldChange)
bioamines_sig_merge = dplyr::left_join(
  bioamines_sig,
  metabolomics_de_patient_kegg[, c("feature_id", "feature_org")],
  by = c("feature_id" = "feature_org")
)
all_kegg_annotated = unique(unlist(compound_kegg@annotation_features))

bioamines_sig_merge = bioamines_sig_merge |>
  dplyr::mutate(
    pathway = dplyr::case_when(
      feature_id.y %in% all_kegg_annotated ~ "in_pathway",
      TRUE ~ "unknown"
    ),
    kegg = dplyr::case_when(
      !is.na(feature_id.y) ~ "in_kegg",
      TRUE ~ "unknown"
    ),
    lfc_direction = dplyr::case_when(
      log2FoldChange > 0 ~ "Positive",
...
list_enrich = list(
  rna_patient_neg = rna_patient_enrichment_reactome$stats$neg |>
    dplyr::filter(padjust <= 0.05) |>
    dplyr::pull(ID),
...
kegg_enrich = list(
  rna_treatment_neg = rna_treatment_enrichment_kegg$stats$neg |>
    dplyr::filter(p <= 0.05) |>
    dplyr::pull(ID),
...
kegg_match = base::intersect(
  kegg_enrich$rna_treatment_neg,
  kegg_enrich$metabolomics_treatment_neg
)
...
go_print_all = go_all |>
  dplyr::filter(grepl(
    "MF:lipoprotein particle binding|CC:vesicle lumen|BP:regulation of tube size|BP:regulation of lipase activity|lipo.*|CC:protein-lipid complex|BP:regulation of spindle checkpoint|BP:regulation of chromosome segregation|BP:negative regulation of cell cycle",
    group
  )) |>
...
reactome_neg = rna_patient_enrichment_grouped_eachreactome$neg |>
  dplyr::filter(!group %in% "") |>
  dplyr::select(-group) |>
