...
b_2 = metabolomics_de_compare |>
	dplyr::filter(type.treatment %in% "bioamines") |>
	ggplot(aes(x = p.value.treatment, y = p.value.patient)) +
	geom_abline(slope = 1, color = "red") +
	geom_point(alpha = 0.5)
...
b_up_1 = metabolomics_de_unpaired_compare |>
	dplyr::filter(type.treatment %in% "bioamines") |>
	...
	geom_point(alpha = 0.5) +
	labs(subtitle = "Bioamines")
b_up_2 = metabolomics_de_unpaired_compare |>
	dplyr::filter(type.treatment %in% "bioamines") |>
...
#| fig-cap: Transcriptomics histogram of log-fold change differnces of treatment (cancer vs normal) and paired patient statistical tests.
rna_de_compare |>
	ggplot(aes(x = (log2FoldChange.treatment - log2FoldChange.patient))) +
	geom_vline(xintercept = 0, color = "red") +
	geom_histogram(bins = 100) +
