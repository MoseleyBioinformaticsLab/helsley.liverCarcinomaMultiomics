...
I think remaining differences in log-fold-changes come down to the handling of missing values in the two cases. 🤷‍♂
...
b_1 = metabolomics_de_compare |>
	dplyr::filter(grepl("bioamine", feature_id)) |>
...
b_2 = metabolomics_de_compare |>
	dplyr::filter(grepl("bioamine", feature_id)) |>
	ggplot(aes(x = -1*log10(pvalue.treatment), y = -1*log10(pvalue.patient))) +
	geom_abline(slope = 1, color = "red") +
	geom_point(size = 0.1) +
	coord_equal()
...
tmp_bioamines = metabolomics_de_unpaired_compare |>
	...
	dplyr::mutate(log2FoldChange_patient = log2FoldChange.treatment,
								...
								p.value_patient = pvalue.treatment,
...
pm_1 = metabolomics_de_compare |>
	dplyr::filter(grepl("pm$", feature_id)) |>
	ggplot(aes(x = log2FoldChange.treatment, y = log2FoldChange.patient)) +
	geom_abline(slope = 1, color = "red") +
	geom_point(size = 0.1) +
...
pm_up_2 = metabolomics_de_unpaired_compare |>
	...
	dplyr::mutate(p.value_patient = pvalue.treatment,
								p.value_unpaired = pvalue.patient) |>
	ggplot(aes(x = -1*log10(p.value_patient), -1*log10(p.value_unpaired))) +
	geom_abline(slope = 1, color = "red") +
	geom_point(size = 0.1) +
	coord_equal()
...
l_1 = metabolomics_de_compare |>
	dplyr::filter(grepl("lipids", feature_id)) |>
	ggplot(aes(x = log2FoldChange.treatment, y = log2FoldChange.patient)) +
	geom_abline(slope = 1, color = "red") +
	geom_point(size = 0.1) +
