...
For significant binomial enrichments of metabolites (p-adjusted < 0.1 for lipids or 0.05 for pathways), genes with significant correlation to the group of metabolites with log-fold-change in the majority direction were extracted, and then binomial enrichment of the genes with significant correlation calculated, as well as for those genes annotated to GO terms.
Binomial enrichment was restricted to those annotations with six or more features.
...
compound_enrich_table = metabolomics_enrichment_reactome_binomial$stats |>
	...
	dplyr::select(description, direction, num_positive, num_negative, p, padjust) |>
	gt::gt() |>
...
For each of these metabolite groups, we can choose those that are in the majority direction, and then find those genes with significant correlations to the metabolites, and calculate binomial enrichments for annotated Gene Ontology terms.

We found that the pathways enriched for the compounds (@tbl-metabolite-binomial-compounds) fell into mostly two groups, disease or metabolism, and subsequently the genes correlated across them and the GO terms enriched for them were also highly shared, so there is not much more analysis there outside of the metabolite - gene correlation heatmap.

I'm actually really curious about the metabolite overlap amongst the various enriched lipid groups, as well as the significantly correlated genes with each of them, and then finally the significant GO terms from binomial enrichment.
...
rna_each = purrr::map(rna_binomial_interesting_lipids$groups$interesting$groups, \(in_group){
...
go_each = purrr::map(rna_binomial_interesting_lipids$go, \(in_group){
	if (is.null(in_group)) {
		return(NULL)
	}
...
null_go = purrr::map_lgl(go_each, is.null)
go_each = go_each[!null_go]
upset_go = ComplexHeatmap::UpSet(ComplexHeatmap::make_comb_mat(go_each))
upset_go
```

These show a whole lot of various things going on, but a lot of the GO terms are heavily shared by the various lipid annotations.

### Compound / Lipid - Gene Correlations
...
compound_show_plot = ComplexHeatmap::Heatmap(compound_map, col = cor_map, name = "Spearman",
																		column_title = "Compound - Gene Correlations",
...
lipid_show_plot = ComplexHeatmap::Heatmap(lipid_map, col = cor_map, name = "Spearman",
																		column_title = "Lipid - Gene Correlations",
