...
rna_sig = rna_de_patient |>
  dplyr::transmute(gene.lfc = sign(log2FoldChange), gene = feature_id)
met_sig = metabolomics_de_patient_list |>
  dplyr::transmute(
    metabolite.lfc = sign(log2FoldChange),
    metabolite = feature_id
  )
sig_cor = dplyr::left_join(
  rna_metabolites_all_spearman_sig,
  rna_sig,
  by = "gene"
)
...
mg_plot = metabolite_to_gene |>
  dplyr::mutate(
    metabolite.lfc = paste0("lfc ", metabolite.lfc),
    sign = paste0("cor ", sign)
  ) |>
  ggplot(aes(x = m2g)) +
  geom_histogram(bins = 100) +
...
all_lfc = dplyr::bind_rows(
  rna_sig |>
    dplyr::transmute(name = gene, lfc = gene.lfc),
  met_sig |>
    dplyr::transmute(name = metabolite, lfc = metabolite.lfc)
)
...
compound_metabolites2 = purrr::map(compound_ids, \(in_pathways) {
  use_groups = rna_binomial_interesting_compounds$groups$interesting$groups[
    in_pathways
  ]
  out_metabolite = purrr::map(use_groups, \(in_group) {
    unique(in_group$metabolite)
  })
  unique(unlist(out_metabolite))
})

out_compound_pathway = unlist(compound_ids[c(
  "disease",
  "metabolism",
  "transport"
)])
...
upset_compounds_smaller = ComplexHeatmap::UpSet(
  make_comb_mat(compound_metabolites2),
...
compound_each = purrr::map(
  rna_binomial_interesting_compounds$groups$interesting$groups,
  \(in_group) {
    unique(in_group$metabolite)
  }
)
...
upset_compounds = ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(compound_each),
...
compound_rna_each_df = purrr::imap(
  rna_binomial_interesting_compounds$groups$interesting$groups,
  \(in_group, group_id) {
    med_cor = in_group |>
  ...
  purrr::list_rbind()
compound_rna_each = split(
  compound_rna_each_df$transcript,
  compound_rna_each_df$id
)
names(compound_rna_each) = reactome_desc2[names(compound_rna_each)]
upset_rna_compound = ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(compound_rna_each),
...
compound_go_each_df = purrr::imap(
  rna_binomial_interesting_compounds$go,
  \(in_group, group_id) {
    if (is.null(in_group)) {
      return(NULL)
    }
...
compound_upset_go = ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(compound_go_each),
...
compound_show_plot = ComplexHeatmap::Heatmap(
  compound_map,
  col = cor_map,
  name = "Spearman",
...
compound_save_plot = ComplexHeatmap::Heatmap(
  compound_map,
  col = cor_map,
  name = "Spearman",
...
lipids_each = purrr::map(
  rna_binomial_interesting_lipids$groups$interesting$groups,
  \(in_group) {
    unique(in_group$metabolite)
  }
)

upset_lipids = ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(lipids_each),
...
upset_rna = ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(rna_each),
...
go_each_df = purrr::imap(
  rna_binomial_interesting_lipids$go,
  \(in_group, group_id) {
    if (is.null(in_group)) {
      return(NULL)
    }
...
upset_go = ComplexHeatmap::UpSet(
  ComplexHeatmap::make_comb_mat(go_each),
...
lipid_gene_dist = hclust(dist((1 - lipid_map))) |> dendsort::dendsort()
lipid_dist = hclust(dist(t(1 - lipid_map))) |> dendsort::dendsort()

lipid_show_plot = ComplexHeatmap::Heatmap(
  lipid_map,
  col = cor_map,
  name = "Spearman",
  column_title = "Lipids",
...
lipid_save_plot = ComplexHeatmap::Heatmap(
  lipid_map,
  col = cor_map,
  name = "Spearman",
  column_title = "Lipids",
