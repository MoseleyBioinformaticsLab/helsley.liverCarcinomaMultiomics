...
metabolites_significant = metabolites_significant |>
...
## Enrichment of All Significantly Correlating Metabolites

We can examine the enrichment for all of the significantly correlated metabolites and see if any of the lipid features show enrichment.

```{r}
#| label: all-significant-enrichment
metabolite_universe = unique(rna_metabolites_spearman$s2)
metabolite_sig = unique(sig_cor$metabolite)

all_metabolite_enrich = hypergeometric_feature_enrichment(
		new("hypergeom_features", significant = metabolite_sig,
				universe = metabolite_universe, annotation = lipid_annotations),
		p_adjust = "BH"
	)
enrich_metabolite_all_df = as.data.frame(all_metabolite_enrich@statistics@statistic_data)
enrich_metabolite_all_df$id = all_metabolite_enrich@statistics@annotation_id
range(enrich_metabolite_all_df$padjust)
```

## Groups of Interesting Genes

We will take some of the interesting groups of gene ontology (GO) terms, the genes annotated to them, and then the significantly correlated metabolites within each group.

### Enrichment

For each group, we look for the significantly correlated metabolites, and then examine various lipid attributes to see if they are present more than expected (hypergeometric), or if their log-fold-changes go in a direction more than expected (binomial).
Unfortunately, there is no enrichment, neither hypergeometric or binomial.

```{r}
#| label: gene-groups-enrichment-hypergeometric
metabolite_universe = unique(rna_metabolites_spearman$s2)
# metabolite_universe = unique(sig_cor$metabolite)

enrich_hypergeom_group = purrr::imap(rna_metabolite_groups_neg, \(in_group, group_id){
	sig_metabolites = unique(in_group$gene_metabolite$metabolite)
	type_enrich = hypergeometric_feature_enrichment(
		new("hypergeom_features", significant = sig_metabolites,
				universe = metabolite_universe, annotation = metabolite_type_annotations),
		p_adjust = "BH"
	)
	lipid_enrich = hypergeometric_feature_enrichment(
		new("hypergeom_features", significant = sig_metabolites,
				universe = metabolite_universe, annotation = feature_lipid),
		p_adjust = "BH"
	)
	type_df = as.data.frame(type_enrich@statistics@statistic_data)
	type_df$id = type_enrich@statistics@annotation_id
	
	lipid_df = as.data.frame(lipid_enrich@statistics@statistic_data)
	lipid_df$id = lipid_enrich@statistics@annotation_id
	lipid_df = lipid_df |>
		dplyr::arrange(padjust)
	list(type = type_df, lipid = lipid_df)
})
```


```{r}
#| label: gene-groups-enrichment-binomial
enrich_binomial_group = purrr::imap(rna_metabolite_groups_neg, \(in_group, group_id){
	positive_fc = metabolites_significant |>
		dplyr::filter(feature_id %in% in_group$gene_metabolite$metabolite, log2FoldChange > 0) |>
		dplyr::filter(feature_id %in% metabolite_universe) |>
		dplyr::pull(feature_id)
	negative_fc = metabolites_significant |>
		dplyr::filter(feature_id %in% in_group$gene_metabolite$metabolite, log2FoldChange < 0) |>
		dplyr::filter(feature_id %in% metabolite_universe) |>
		dplyr::pull(feature_id)
	
	lipid_binomial_enrich = binomial_feature_enrichment(
		new("binomial_features", positivefc = positive_fc, negativefc = negative_fc,
				annotation = feature_lipid),
		p_adjust = "BH", min_features = 6
	)
	if (!is.null(lipid_binomial_enrich)) {
		lipid_df = as.data.frame(lipid_binomial_enrich@statistics@statistic_data)
		lipid_df$id = lipid_binomial_enrich@statistics@annotation_id
		lipid_df = lipid_df |> dplyr::arrange(padjust)
		return(lipid_df)
	} else {
		return(NULL)
	}
...
### Metabolites With Significant Correlations

OK, what metabolites are significantly correlated with each gene group?
When I examined these tables, there were very few named metabolites.
That seems to be a general problem with the significant metabolites, maybe?

```{r}
#| label: gene-group-metabolite-tables
gene_group_metabolite_tables = purrr::imap(rna_metabolite_groups_neg, \(in_group, group_id){
	tmp_metabolites = metabolites_significant |>
		dplyr::filter(feature_id %in% in_group$gene_metabolite$metabolite) |>
		dplyr::select(feature_id, annotation, metabolite_name, log2FoldChange, pvalue, padj) |>
		dplyr::arrange(log2FoldChange, padj) |>
		dplyr::mutate(gene_group = group_id)
	tmp_metabolites
})
```

## Groups of Interesting Metabolites

Finally, we can perform binomial enrichment of the log-fold-change of annotated metabolites, look for significantly enriched groups, and then extract the significantly correlated genes with those groups of metabolites, and subsequently look at the binomial enrichment of the genes as well.

For the metabolites, we primarily 

@tbl-metabolite-binomial lists the 

```{r}
#| label: tbl-metabolite-binomial
