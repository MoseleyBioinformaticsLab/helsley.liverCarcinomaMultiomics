...
sig_matrix = matrix(0, nrow = length(unique(sig_cor$metabolite)),
										ncol = length(unique(sig_cor$gene)))

rownames(sig_matrix) = unique(sig_cor$metabolite)
colnames(sig_matrix) = unique(sig_cor$gene)
for (irow in seq_len(nrow(sig_cor))) {
	sig_matrix[sig_cor[irow, "metabolite"], sig_cor[irow, "gene"]] = sig_cor[irow, "cor"]
}
```

```{r}
#| label: fig-cluster-metabolites
metabolite_clusters = dbscan(sig_matrix, eps = 1)
met_cluster_df = data.frame(name = rownames(sig_matrix), cluster = metabolite_clusters$cluster) |>
	dplyr::mutate(cluster2 = paste0("met.", cluster))

gene_clusters = dbscan(t(sig_matrix), eps = 1)
