...
```{r}
#| label: fig-rna-seq
#| fig-cap: RNA-seq samples PCA (left) and differential gene heatmap (right). Differential genes are colored by the log2-fold-change cancerous / normal adjacent samples in each patient.
rna_hm_grob = draw(rna_patient_heatmap) |> grid.grabExpr()

(refactor_treatment(rna_qcqa) |>
	ggplot(aes(x = PC1, y = PC2, color = Treatment)) +
	geom_polygon(stat = "ellipse", aes(color = Treatment), fill = NA) +
	geom_point(size = 2) +
	scale_color_manual(values = use_colors) +
	theme(legend.position = "inside", legend.position.inside = c(0.6, 0.8)) +
	labs(x = rna_qcqa$pca_nooutlier_variance$labels[1],
		 y = rna_qcqa$pca_nooutlier_variance$labels[2])) | rna_hm_grob
```
...
lipidomics_hm_grob = draw(lipidomics_patient_heatmap) |> grid.grabExpr()
...
(refactor_treatment(lipidomics_qcqa) |>
	...
	labs(x = lipidomics_qcqa$pca_nooutlier_variance$labels[1],
		 y = lipidomics_qcqa$pca_nooutlier_variance$labels[2])) | lipidomics_hm_grob
```
...
bioamines_hm_grob = draw(bioamines_patient_heatmap) |> grid.grabExpr()
...
(refactor_treatment(bioamines_qcqa) |>
  ...
  geom_polygon(stat = "ellipse", aes(color = Treatment), fill = NA) +
	...
	labs(x = bioamines_qcqa$pca_nooutlier_variance$labels[1],
		 y = bioamines_qcqa$pca_nooutlier_variance$labels[2])) | bioamines_hm_grob
```
...
pm_hm_grob = draw(pm_patient_heatmap) |> grid.grabExpr()
(refactor_treatment(primary_metabolism_qcqa) |>
	...
	labs(x = primary_metabolism_qcqa$pca_nooutlier_variance$labels[1],
		 y = primary_metabolism_qcqa$pca_nooutlier_variance$labels[2])) | pm_hm_grob
```
