...
For the determination of differential features in tumor vs non-tumor between females and males, a secondary variable was created to handle the confounding of sex and patient (sex_rep, as noted in the DESeq2 vignette).
The design used is then ~ sex + sex:treatment + sex:sex_rep, with a list contrast of "sexf.treatmentcancerous" and "sexm.treatmentcancerous" to extract the comparison of $\text{female}(\text{cancer} - \text{non-cancer}) - \text{male}(\text{cancer} - \text{non-cancer})$.
